...
class TensorCore_MixedPrecision_multslot_simple(DimM: Int, DimN: Int, DimK: Int,slot_num:Int = 1, xDatalen:Int = 16, tcctrl:TCCtrl_mulslot_v2) extends Module {
  //  mnk defined as cuda.
  //  m8n8k8
  ...
  val TCComputation = Module(new TC_ComputationArray_MixedPrecision(16,8,4,4,tcCtrl = tcctrl_mslot))
...
//  val reg_ismix = RegNext(io.in.bits.ctrl.isMixedPrecisionMode)
  ...
  //A 8*8 row
  for (i <- 0 until 64/2) TCComputation.io.in.bits.A(i) := 0.U
  //B 8*4 col
  for (i <- 0 until 32/2) TCComputation.io.in.bits.B(i) := 0.U
  //C 8*4 row First
  ...
  switch(recvNS(TCComputation.io.out.bits.ctrl.sel_slot_num)) {
    ...
    is(1.U) {
      when(TCComputation.io.out.bits.ctrl.isMixedPrecisionMode) {
        ...
        for (m <- 0 until DimN * DimM / set_num/2) {
