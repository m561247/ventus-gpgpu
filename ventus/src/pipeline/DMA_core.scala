...
// import mmu.SV32.asidLen
 ...
 class regSave extends Bundle{  //num_thread
...
//   val p_addr_reg = RegInit(0.U(paLen.W))
  ...
  TensorVars.datawidth := Mux((reg_save.in1(0).asUInt === UINT64.asUInt)||(reg_save.in1(0).asUInt ===INT64.asUInt)||(reg_save.in1(0).asUInt === FP64.asUInt), 8.U,  //Byte
    Mux((reg_save.in1(0).asUInt === INT32.asUInt) || (reg_save.in1(0).asUInt === UINT32.asUInt) || (reg_save.in1(0).asUInt === FP32.asUInt)|| (reg_save.in1(0).asUInt === FP32_FTZ.asUInt) || (reg_save.in1(0).asUInt === TF32.asUInt)|| (reg_save.in1(0).asUInt === TF32_FTZ.asUInt), 4.U,
      Mux((reg_save.in1(0).asUInt === INT16.asUInt) || (reg_save.in1(0).asUInt === UINT16.asUInt) || (reg_save.in1(0).asUInt === FP16.asUInt) || (reg_save.in1(0).asUInt === BF16.asUInt), 2.U, 1.U ) )  )
  ...
  io.to_l2cache.bits.a_addr := Cat(reg_save.address(xLen - 1, xLen - 1 - addr_tag_bits + 1), 0.U((xLen - addr_tag_bits).W))     //dcache_BlockWords: Int = num_thread//16    BytesOfWord//4
...
//   io.from_l2TLB.ready := state === s_tlb_rsp
  ...
  //  printf("l2req:address: %b\n",2io.to_l2cache.bits.a_addr.asUInt)
}
      ...
      switch(output_inst.funct){
        is(0.U) {
          (0 until (numgroupl2cache)).foreach(x => {  // numgroupl2cache =(number_thread*4= 16*4(word_size=4 Byte) )  / 4  =64 /4 =16
            ...
            var condition2 = (tag_wire.asUInt + (x.asUInt * dma_aligned_bulk.asUInt) >= output_inst.src) && ((tag_wire.asUInt + ((x + 1).asUInt * dma_aligned_bulk.asUInt - 1.U)) <= output_inst.src + output_inst.copysize - 1.U )
            var condition3 = (tag_wire.asUInt + (x.asUInt * dma_aligned_bulk.asUInt)) <= output_inst.src + output_inst.copysize- 1.U && (tag_wire.asUInt + ((x.asUInt + 1.U) * dma_aligned_bulk.asUInt) - 1.U) >= output_inst.src + output_inst.copysize - 1.U
        ...
        is(1.U){
          ...
          (0 until (numgroupl2cache)).foreach(x => {
            var condition2 = (tag_wire.asUInt + (x.asUInt * dma_aligned_bulk.asUInt) >= output_inst.src) && (tag_wire.asUInt + (x.asUInt * dma_aligned_bulk.asUInt) < output_inst.src + output_inst.copysize - 1.U)
            when(condition2) {
              printf(p"addr_true : 0x${Hexadecimal(tag_wire.asUInt + (x.asUInt * dma_aligned_bulk.asUInt))} ")
              mask_l2cache(x) := true.B
            }.otherwise {
