...
class single (implicit param: Parameters)extends AnyFreeSpec with ChiselScalatestTester{
  "first_test" in {
    test(new pipe()(param)).withAnnotations(Seq(WriteVcdAnnotation)) { div =>
      //c.io.in1.poke(2.U)
...
class AdvancedTest extends AnyFreeSpec with ChiselScalatestTester{ // Working in progress
  ...
  "adv_test2" in {
    ...
    test(new GPGPU_SimWrapper(FakeCache = false)).withAnnotations(Seq(CachingAnnotation, VerilatorBackendAnnotation,WriteVcdAnnotation)){ c =>
